...
def get_model(config, tokenizer=None, **kwargs):
    ...
    # Support for base_model_name (for loading checkpoints into base models)
    ...
    # Determine which path to use for model loading
    # If model_path is a .ckpt file, load base model first then apply checkpoint
    load_path = base_model_name if base_model_name else model_path
    is_checkpoint = model_path and model_path.endswith('.ckpt')
    ...
    if is_checkpoint and os.path.exists(model_path):
        logging.warning(f"Loading checkpoint from {model_path}...")
        ...
        logging.warning("Checkpoint loaded successfully")
...
def get_tokenizer(config, **kwargs):
    model_path = config.model_path
    base_model_name = getattr(config, 'base_model_name', None)
    # Use base_model_name for tokenizer if model_path is a checkpoint file
    tokenizer_path = base_model_name if (base_model_name and model_path.endswith('.ckpt')) else model_path
    logging.warning(f"Loading tokenizer from {tokenizer_path}...")
...
class TrainDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, shuffle: bool = True, shuffle_seed: int = 42, load_idx_list = None, begin_id = None, end_id = None, use_lm_format: bool = False):
        ...
        if use_lm_format:
            # For language models: use input as context, output as continuation
            sources = [example.get('input', '') for example in list_data_dict]
            targets = [f"{example['output']}{tokenizer.eos_token}" for example in list_data_dict]
        else:
            # For instruction-tuned models: use instruction template
            sources = [
...
class TestDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, use_lm_format: bool = False):
