...
def MP_run_calc_infulence_function(rank, world_size, process_id, config, mp_engine, restart = False):
    ...
    # Check if we should use simple LM format (for language models like OPT)
    use_lm_format = getattr(config.data, 'use_lm_format', False)

    train_dataset = TrainDataset(config.data.train_data_path, tokenizer, shuffle=False, begin_id=config.data.begin_id, end_id=config.data.end_id, use_lm_format=use_lm_format)
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=False, num_workers=0)
    print(f"CUDA {rank}: Training Datalodaer loaded!")

    test_dataset = TestDataset(config.data.test_data_path, tokenizer, use_lm_format=use_lm_format)
    ...
    def get_s_test_vec_list(model):
        ...
        # Check if we should load pre-computed s_test vectors
        s_test_path = getattr(config.influence, 's_test_path', None)
        load_s_test = getattr(config.influence, 'load_s_test', False)
        save_s_test = getattr(config.influence, 'save_s_test', False)

        if load_s_test and s_test_path and os.path.exists(s_test_path):
            print(f"Loading pre-computed s_test vectors from {s_test_path}")
            s_test_data = torch.load(s_test_path, map_location=f"cuda:{rank}")
            s_test_vec_list = s_test_data['s_test_vec_list']
            print(f"Loaded {len(s_test_vec_list)} s_test vectors")
        ...
        # Save s_test vectors if requested
        if save_s_test and s_test_path:
            print(f"Saving s_test vectors to {s_test_path}")
            os.makedirs(os.path.dirname(s_test_path), exist_ok=True)
            # Move to CPU for saving to avoid GPU memory issues
            s_test_cpu = [v.cpu() for v in s_test_vec_list]
            torch.save({'s_test_vec_list': s_test_cpu}, s_test_path)
            print(f"Saved {len(s_test_vec_list)} s_test vectors")
    ...
    with mp_engine.gpu_locks[rank].get_lock():
        ...
        if config.influence.delete_model and not config.influence.cal_words_infl:
            del model, oporp_eng
            gc.collect()
            ...
            if cal_word_infl < 0:
                ...
                for i in range(len(test_dataset)):
                    ...
                    if influence != influence: # check if influence is Nan
                        print(f'Warning: Got NaN influence for sample {idx}, setting to 0')
                        influence = np.float64(0.0)
                    mp_engine.result_q.put((i, idx, real_id, influence), block=True, timeout=None)

            else:
                # Pass RapidGrad engine for dimension matching if enabled
                rapidgrad_k = config.influence.RapidGrad.RapidGrad_K if config.influence.RapidGrad.enable else None
                _, words_influence = grad_z(z, t, input_len, model, gpu=rank, return_words_loss=True, s_test_vec=s_test_vec_list[cal_word_infl], rapidgrad_eng=oporp_eng, rapidgrad_k=rapidgrad_k)
    ...
    while True:
        ...
        if not config.influence.skip_influence and ((i + 1)%(total_size//50) == 0 or i == total_size - 1):
            for j in range(test_dataset_size):
                ...
                infl = [ x.tolist() if not isinstance(x, (int, float)) else x for x in infl_list[j] ]
